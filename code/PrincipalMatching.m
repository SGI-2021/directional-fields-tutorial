...
GaussCurvature = zeros(length(V),1);

%Confidence check: Gaussian Curvature sums up to the (2pi*) Euler characteristic
eulerChar = length(V)-length(EV)+length(F);
angleDefectError = abs(sum(GaussCurvature)-2*pi*eulerChar)

%plotting Gaussian curvature
figure
hold on
patch('faces', F, 'vertices', V,  'faceColor', 'interp', 'CData', GaussCurvature); axis equal; cameratoolbar;
